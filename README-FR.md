...
*AMCtoOwncloud* est un *script Nautilus* qui permet d'envoyer simplement les copies corrigées avec *[Auto Multiple Choice](http://auto-multiple-choice.net/)* (*AMC*) sur un serveur *Owncloud/Nextcloud*.
...
Évidemment, les noms `Contrôles/`, `Interro 1`, et `Interros Maths` sont modifiables, et les autres champs sont extraits du fichier `.csv` des étudiants. Par exemple, si vous n'avez pas de classes/groupes dans votre fichier `.csv`, et si vous dossier racine vide `''`, l'organisation des fichiers sera: 
...
Saisir votre mot de passe *Owncloud* , le nom du contrôle, et attendre :

<img src="/docs/UsingScript2-Output.png" width="800x">

Les éventuels problèmes recontrés devraient être indiqué (erreurs d'identification, d'envoi ou de partage, fichiers non associés à des étudiants).
...
Vérifier que `gnome-terminal` est installé ou éditer le script `AMCtoOwncloud.sh` pour utiliser un autre terminal.
...
Le fichier `.csv` contenant les informations des étudiants doit utiliser des points-virgules `;` comme séparateurs les titres de colonnes suivants (il y a des paramètres optionnels dans la méthode `identify_students()` pour personnaliser ce comportement, voir [plus bas](https://github.com/remjg/AMCtoOwncloud/blob/master/README.md#special-use-case)):
...
Enfin, **les copies corrigées doivent comporter le numéro d'étudiant** dans leur nom
(le premier nombre du nom de fichier est extrait pour associer chaque copie à l'étudiant correspondant). Pour ce faire, ne pas oublier de configurer *auto-multiple-choice* avec les titres des colonnes du fichier `.csv` :

<img src="/docs/RenamingAnnotatedPapers.png" width="400x">

## Cas particuliers d'utilisation

Pour changer le comportement du script, vous pouvez éditer les 4 dernières lignes du fichier `.AMCtoOwncloud.py` :
...
Par exemple, si votre serveur *Owncloud* se trouve derrière un *portail d'autentification unique*, vous pouvez utiliser l'option `SSO=True`. Testé avec un espace numérique de travail [Envole](https://envole.ac-dijon.fr) qui utilise CAS comme portail d'autentification unique.

Vous pouvez aussi sauvegarder les liens partagés dans le fichier `.csv` courant avec l'option `replace_csv=True`. Pensez à faire une sauvegarde avant et notez que *les lignes commençant par un `#` (commentaires) seront perdu lors du processus*.

Plus d'options sont disponibles, vous pouvez trouver ci-dessous une liste complète de toutes les optiopns avec les paramètres par défaut :
